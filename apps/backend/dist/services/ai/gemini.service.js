...
const SYSTEM_PROMPT = `You are Rahul, a highly experienced Senior B2B Sales Executive at ${company_1.COMPANY.name}, a leading IT solutions company based in Nagpur, Maharashtra. 

COMPANY PROFILE:
- Company: ${company_1.COMPANY.name}
- Owner: ${company_1.COMPANY.ownerName} (${company_1.COMPANY.designation})
- Location: ${company_1.COMPANY.address}
- Phone: ${company_1.COMPANY.phone1} / ${company_1.COMPANY.phone2}
- Email: ${company_1.COMPANY.email}
- GSTIN: ${company_1.COMPANY.gstin}
...
// ─── Main Chat Function ────────────────────────────────────────────────────
async function sendAIMessage(userMessage, conversationHistory = []) {
    try {
        // Check Knowledge Base for relevant context
        const ragContext = await (0, rag_service_1.searchKnowledgeBase)(userMessage);
        ...
        logger_1.logger.error('Gemini AI error:', error);
        return {
...
Respond ONLY in this exact JSON format:
{
  ...
  "detectedIntent": "<HIGH|MEDIUM|LOW>"
        ...
        logger_1.logger.error('Lead qualification error:', error);
        return { score: 30, temperature: 'COLD', reasoning: 'Qualification failed', qualificationData: {} };
    }
}
// ─── Generate Follow-Up Message ────────────────────────────────────────────
async function generateFollowUpMessage(leadData) {
    try {
        const model = genAI.getGenerativeModel({
            model: 'gemini-2.5-pro',
...
From: ${company_1.COMPANY.ownerName}, ${company_1.COMPANY.name}
...
- End with contact details: ${company_1.COMPANY.phone1}
...
Do NOT use Hindi, Hinglish, or informal language.`;
        ...
        logger_1.logger.error('Follow-up generation error:', error);
        return `Dear ${leadData.name},\n\nI hope you are doing well. I wanted to follow up regarding your inquiry with ASN Expo. We would love to assist you with your IT requirements.\n\nPlease feel free to reach out at ${company_1.COMPANY.phone1} or reply to this message.\n\nBest regards,\n${company_1.COMPANY.ownerName}\n${company_1.COMPANY.name}`;
    }
}
// ─── Auto Reply for New Inquiry ────────────────────────────────────────────
async function generateAutoReply(inquiry, customerName) {
    try {
        const model = genAI.getGenerativeModel({
            model: 'gemini-2.5-pro',
...
Respond ONLY with the message text.`;
        ...
        logger_1.logger.error('Auto reply generation error:', error);
        return `Thank you for your inquiry, ${customerName || 'valued customer'}! We at ${company_1.COMPANY.name} are glad to assist you. Could you please share the quantity required and your approximate budget? This will help us provide you the best recommendation and a detailed quotation.`;
    }
}
// ─── Product Recommendation ────────────────────────────────────────────────
async function recommendProducts(requirement, budget, quantity) {
    const model = genAI.getGenerativeModel({
        model: 'gemini-2.5-pro',
        systemInstruction: SYSTEM_PROMPT,

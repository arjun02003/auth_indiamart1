...
import { generateQuotationPDF } from '../services/pdf/quotation.service';
import { sendQuotationEmail } from '../services/email/email.service';
...
export const getQuotationById = async (req: Request, res: Response): Promise<void> => {
  try {
    const quotationId = getRouteParam(req.params.id);
    if (!quotationId) { res.status(400).json({ error: 'Quotation id is required' }); return; }
...
export const createQuotation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let subtotal = 0;
    let taxAmount = 0;
    const processedItems = items.map((item: { name: string; description?: string; quantity: number; unitPrice: number; taxRate?: number; productId?: string }) => {
...
export const sendQuotationByEmail = async (req: Request, res: Response): Promise<void> => {
  try {
    const quotationId = getRouteParam(req.params.id);
    if (!quotationId) { res.status(400).json({ error: 'Quotation id is required' }); return; }
    ...
    if (!email) { res.status(400).json({ error: 'No email address for this lead/customer' }); return; }
    ...
    await prisma.activity.create({
      data: {
        ...
        leadId: quotation.leadId, userId: 'system',
      },
    });
...
export const updateQuotationStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (status === 'ACCEPTED') {
      await prisma.lead.update({ where: { id: quotation.leadId }, data: { status: 'CONVERTED', convertedAt: new Date() } });
    }

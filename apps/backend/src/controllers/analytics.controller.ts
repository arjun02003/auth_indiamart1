...
export const getAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [
      leadsByStatus, leadsBySource, leadsByTemperature,
      quotationsByStatus, revenueByMonth,
      conversionFunnel, topProducts,
    ] = await Promise.all([
      prisma.lead.groupBy({ by: ['status'], _count: { id: true } }),
      prisma.lead.groupBy({ by: ['source'], _count: { id: true } }),
      prisma.lead.groupBy({ by: ['temperature'], _count: { id: true } }),
      prisma.quotation.groupBy({ by: ['status'], _count: { id: true }, _sum: { grandTotal: true } }),
